...
export default {
  ...
  methods: {
    getDetail() {
      this.$api.xiaoBanKe.CourseCategory.Edit(this.id).then((res) => {
    ...
    confirm() {
      ...
      this.loading = true;
      if (this.text === "添加分类") {
        this.$api.xiaoBanKe.CourseCategory.Store(this.form)
          .then(() => {
            this.loading = false;
        ...
        this.$api.xiaoBanKe.CourseCategory.Update(this.id, this.form)
          .then(() => {
            this.loading = false;
            this.$message.success(this.$t("common.success"));
            this.$emit("success");
