...
export default {
  ...
  methods: {
    getDetail() {
      this.$api.Course.Topic.Category.Detail(this.id).then((res) => {
        var data = res.data;
    ...
    confirm() {
      ...
      this.loading = true;
      if (this.text === "添加分类") {
        this.$api.Course.Topic.Category.Store(this.form)
          .then(() => {
            this.loading = false;
        ...
        this.$api.Course.Topic.Category.Update(this.id, this.form)
          .then(() => {
            this.loading = false;
            this.$message.success(this.$t("common.success"));
            this.$emit("success");
